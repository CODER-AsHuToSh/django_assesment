...
class TodoItemIntegrationTests(TestCase):
    ...
    def test_retrieve_todo_item(self):
        response = self.client.get(reverse('todo-read', kwargs={'pk': self.todo_item.pk}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_todo_item(self):
        ...
        response = self.client.put(reverse('todo-update', kwargs={'pk': self.todo_item.pk}), update_data)
        self.assertEqual(response.status_code, 200)
        # Add assertions to check if the item was updated correctly

    def test_delete_todo_item(self):
        response = self.client.delete(reverse('todo-delete', kwargs={'pk': self.todo_item.pk}))
